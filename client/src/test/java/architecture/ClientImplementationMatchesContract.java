...
/** Checks the name-derived core contract of a client implementation. */
final class ClientImplementationMatchesContract extends ArchCondition<JavaClass> {

    private static final String IMPLEMENTATION_PACKAGE_SUFFIX = ".client";
    private static final String IMPLEMENTATION_NAME_SUFFIX = "Client";
    private static final String SERVICE_NAME_SUFFIX = "ServiceClient";

    private final JavaClasses contractClasses;

    ClientImplementationMatchesContract(final JavaClasses contractClasses) {
    ...
    public void check(final JavaClass implementation, final ConditionEvents events) {
        ...
        if (!simpleName.endsWith(SERVICE_NAME_SUFFIX)) {
            events.add(SimpleConditionEvent.violated(
                    implementation,
                    implementationName + " does not have a valid ServiceClient suffix, so its core contract cannot be derived"));
            return;
        }
        ...
        if (!packageName.endsWith(IMPLEMENTATION_PACKAGE_SUFFIX)) {
            events.add(SimpleConditionEvent.violated(
                    implementation,
                    implementationName + " is not in a direct service.client package, so its core contract cannot be derived"));
            return;
        }
